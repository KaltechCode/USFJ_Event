...
Use a VPS or hosting plan that explicitly supports a persistent Node.js 22 process and HTTPS reverse proxy. Ask ScalaHosting support to confirm this for your specific plan. A static upload to public_html is not enough: the registration and organizer endpoints require Node.js.
...
2. Open SQL Editor and execute `supabase/001_initial.sql` once. This creates durable rate-limit counters. It is an initial migration, not a script to rerun on existing tables. Then run `supabase/002_registration_forms.sql` once. Projects that ran an earlier version of `001_initial.sql` should also run `supabase/003_remove_invitations_and_rsvps.sql` once to drop the retired invitations and RSVP tables.
...
3. Submit one test volunteer, guest and sponsor form using distinct test identities. Confirm each in Supabase and /admin. Re-submit to verify duplicates are blocked.
...
5. Sign in as the allowlisted organizer and check registration counts, search and CSV export.
6. Confirm the publishable key cannot directly read registration rows; use verify-security.sql to inspect grants.
...
This package does NOT copy any existing Sites/D1 responses into Supabase, and does not change or disable the Sites URL. Check whether responses already exist before switching. If they do, export them through the authorized organizer/database controls, map them to the PostgreSQL schema and reconcile counts before cutover. Preserve record identifiers where compatible. Do not publish two guest links to separate databases without a plan to reconcile responses.
...
Browser → same-origin Next.js API → Supabase PostgreSQL/Auth. Guests never access the database directly. RLS and revoked anon/authenticated grants deny browser access; server operations use a secret key after validation/authorization. Database uniqueness protects duplicate submissions, including concurrent requests. Registration request IDs make safe retries idempotent.

The organizer dashboard fetches batches of 500 to avoid Supabase's default result-size cap. It refuses partial exports above 20,000 records.

Rate limits: 30 registration attempts or 10 login attempts per IP per 15 minutes. Counters persist in Supabase and expire through cleanup in the rate-limit function. Configure the reverse proxy correctly so guests do not all share one apparent IP. No rate-limiter data or public guest read endpoint is exposed.
